...
return {
conversation: 'WhatsApp Bot By Supreme'
}}
}

const Tkm = func.makeWASocket(connectionOptions)
if (usePairingCode && !Tkm.authState.creds.registered) {
const phoneNumber = await question(chalk.cyan.bold('Enter the WhatsApp number starting with 254\nExample : 254xxxx\n'))
...
let teksnotif = `*JUNE MD* Successfully Connected to WhatsApp   ${Tkm.user.id.split(":")[0]}`
Tkm.sendMessage("254798570132@s.whatsapp.net", {text: teksnotif})
console.log(chalk.cyan.bold('𝐉ᴜɴᴇ 𝐌ᴅ  Successfully Connected'))
...
if (ff.isGroup == false) {
if (ff.status == "offer") {
let sendcall = await Tkm.sendMessage(ff.from, {text: `@${ff.from.split("@")[0]} _sorry Anticall activated,Blocking you_`, contextInfo: {mentionedJid: [ff.from], externalAdReply: {showAdAttribution: true, thumbnail: fs.readFileSync("./media/warning.jpg"), title: "｢ CALL DETECTED ｣", previewType: "PHOTO"}}}, {quoted: null})
...
ppuser = 'https://files.catbox.moe/0vjp1q.jpg'
}
if (anu.action == 'add') {
...
contextInfo: {mentionedJid: [...tag], externalAdReply: { thumbnailUrl: ppuser, title: '© Welcome Message', body: '', renderLargerThumbnail: false, sourceUrl: linkgc, mediaType: 1}}})
