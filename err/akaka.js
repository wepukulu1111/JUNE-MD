...
if (m.isGroup && global.antigroup[m.chat]) {
  ...
  if (regex.test(m.text)) {
    if (isGroupAdmins || m.key.fromMe) return; // kalau yang kirim admin/owner, abaikan

    reply(`⚠️ Link grup terdeteksi!\nPengguna: @${m.sender.split('@')[0]}`, [m.sender]);

    // Kick user (jika bot admin)
    if (isBotGroupAdmins) {
...
if (!isCreator) return reply("Khusus owner bre!");
 ...
 let output = match[1].trim(); // ambil hanya isi blok case

 if (output.length > 4000) output = output.slice(0, 4000) + '\n... (terpotong)';

 reply(output); // kirim polos, tanpa backtick, tanpa format
}
break
...
case 'editcase': {
  ...
  // Bungkus isi baru dengan format case yang utuh
  const kodeBaru = `\ncase '${target}' : {\n${newCode.trim()}\nbreak`;
...
if (!isCreator) return reply("Khusus owner bre!");
 ...
 // Bikin file catatan.txt kalau belum ada
...
case 'tourl': {
 ...
 let { upload } = require('./lib/uploader'); // pastikan lo punya uploader
